...
if(CUSTOMER=="1111" || HANDLER=="JETEX"){ //////// HANDLING QUOTATION JETEX EAM
...
if(CUSTOMER=="999" && HANDLER!="JETEX"){ ////////////// QUOTATION HANDLING ROJ
...
if((CUSTOMER!="1111" && CUSTOMER!="999" && HANDLER!="JETEX") || HANDLER=="NIL"){  ////////////// START OF STANDARDIZED HANDLING QUOTATION 
if ((M<=7) && (M>0)){ 
  Concession=30; 
  Handling=169;
  BasicFee=180;
...
////////////////////////
if(HANDLER=="NIL" && CUSTOMER=="1111"){Concession=550; Handling=0; Concession_handler=0;} ;
if(HANDLER=="JETEX"){
  // Concession_handler=0;
  // Handling=0;
}
...
const sql=`UPDATE flight SET Concession=?,Handling=?,BasicFee=?,Concession_handler=?, QCM_OK=?, flight_number= ?,MTD_PAY= ?,ACFT= ?,MTOW= ?,TTL_CREW= ?,NAT_FLT= ?,ARV_DTE= ?,PROV= ?,DEST= ?,PAX_IN= ?,PAX_OUT= ?,CGO_IN= ?,CGO_OUT= ?,EA= ?,ED= ?,PKG= ?,DLY_CODE= ?,DLY_TIME= ?,DLY_REASON= ?,INF_IN_OUT= ?,LIGHTING= ?,COMMENT= ?,PUSH_BACK= ?,HANDLER= ?,DSK_USED= ?,CHK_IN_OPEN= ?,CHK_IN_CLOSE= ?,BOARD_OPEN= ?,BOARD_CLOSE= ?,
DOORS_SHUT= ?,COMMENT_CHK_IN= ?,COMMENT_BOARD= ?,INFO_GIVEN_PAX= ?,PAX_REACTION= ?,EXP_INCURRED= ?,OTHER_GSE= ?,OPERATOR= ?,PROVIDER= ?,COMMENT_CLEAN= ?,COMMENT_RAMP= ?,COMMENT_CGO= ?,COMMENT_CATERING= ?,EXP_AUTH_BY= ?,COMMENT_CREW= ?,WO_HNDL_INV= ?,APT_FEES_INV= ?,APT_FEES_AMT= ?,APT_FEES_PAY= ?,APT_FEES_RMK= ?,TASPT_INV= ?,TASPT_AMT= ?,TASPT_PAY= ?,
TASPT_RMK= ?,VIP_ARV_INV= ?,VIP_ARV_PAX_NBR= ?,VIP_ARV_PAY= ?,VIP_ARV_RMK= ?,VIP_ARV_PRO= ?,VIP_DEP_INV= ?,VIP_DPT_PAX_NBR= ?,VIP_DEP_PAY= ?,VIP_DEP_RMK= ?,VIP_DEP_PRO= ?,CATERING_INV= ?,CATERING_AMT= ?,CATERING_PAY= ?,CATERING_RMK= ?,CATERING_PRO= ?,CREW_TRS_INV= ?,CREW_TRS_AMT= ?,CREW_TRS_PAY= ?,CREW_TRS_RMK= ?,CREW_TRS_PRO= ?,PAX_TRS_INV= ?,PAX_TRS_AMT= ?,PAX_TRS_PAY= ?,PAX_TRS_RMK= ?,
PAX_TRS_PRO= ?,CREW_HTC_INV= ?,CREW_HTC_AMT= ?,CREW_HTC_PAY= ?,CREW_HTC_RMK= ?,CREW_HTC_PRO= ?,PAX_HTC_INV= ?,PAX_HTC_AMT= ?,PAX_HTC_PAY= ?,PAX_HTC_RMK= ?,PAX_HTC_PRO= ?,FUEL_INV= ?,FUEL_AMT= ?,FUEL_PAY= ?,FUEL_RMK= ?,FUEL_PRO= ?,CGO_INV= ?,CGO_AMT= ?,CGO_PAY= ?,CGO_RMK= ?,CGO_PRO= ?,TECH_INT_INV= ?,TECH_INT_AMT= ?,TECH_INT_PAY= ?,TECH_INT_RMK= ?,TECH_INT_PRO= ?,LND_PMT_AMT= ?,LND_PMT_RMK= ?,TRVL_EXP_AMT= ?,TRVL_EXP_RMK= ?,PORTERS_AMT= ?,PORTERS_RMK= ?,PRNT_PAGES_AMT= ?,PRNT_PAGES_RMK= ?,PHONE_COM_AMT= ?,PHONE_COM_RMK= ?,OTHER_CHG1= ?,OTHER_CHG1_INV= ?,OTHER_CHG1_AMT= ?,OTHER_CHG1_PAY= ?,OTHER_CHG1_RMK= ?,OTHER_CHG1_PRO= ?,OTHER_CHG2= ?,OTHER_CHG2_INV= ?,OTHER_CHG2_AMT= ?,OTHER_CHG2_PAY= ?,OTHER_CHG2_RMK= ?,OTHER_CHG2_PRO= ?,CURRENC_EX_BUY= ?,CURRENC_EX_SELL= ?,SUPERVISION_ARV_BY= ?,APPROVED_BY= ?,RECEIVED_AT= ?,SUP_CHARGE_NOTE_AMT= ?,SUP_CHARGE_NOTE_INV= ?,SUP_CHARGE_NOTE_RMK= ?,EXCESS_BAG= ?,VERSION= ?,MOTIF= ?,DATE_SUP= ?,TRS_APT_INV= ?,TRS_APT_PRO= ?,TRS_APT_PAY= ?,TRS_APT_AMT= ?,TRS_APT_RMK= ?,SUPERVISION_DEP_BY= ?,GPU= ?,GPU_TIME= ?,ASU= ?,flight_number_arv= ?,OTHER_CHG3= ?,OTHER_CHG3_INV= ?,OTHER_CHG3_AMT= ?,OTHER_CHG3_PAY= ?,OTHER_CHG3_RMK= ?,OTHER_CHG3_PRO= ?,OTHER_CHG4= ?,OTHER_CHG4_INV= ?,OTHER_CHG4_AMT= ?,OTHER_CHG4_PAY= ?,OTHER_CHG4_RMK= ?,OTHER_CHG4_PRO= ?,CORVER_BY= ?,CUTE_INV= ?,CUTE_AMT= ?,CUTE_PAY= ?,CUTE_RMK= ?,AMBU_NBR= ?,WCH_NBR= ?,UMNR_NBR= ?,FAE_LINK= ?,APT_FEES_LINK= ?,CATERING_LINK= ?,APT_TRANS_LINK= ?,CREW_TRANS_LINK= ?,PAX_TRANS_LINK= ?,FUEL_LINK= ?,CREW_HOTAC_LINK= ?,PAX_HOTAC_LINK= ?,CGO_FEES_LINK= ?,TECH_INTER_LINK= ?,FACTURETTES_LINK= ?,TASPT_LINK= ?,CUTE_LINK= ?,QSR_LINK= ?,ROJ_WO_LINK= ?,IMOC_FORM_LINK= ?,IMOC_HDLG_LINK= ?,Service_Type= ?,OTHER_CHG5= ?,OTHER_CHG5_INV= ?,OTHER_CHG5_AMT= ?,OTHER_CHG5_PAY= ?,OTHER_CHG5_RMK= ?,OTHER_CHG5_PRO= ?,MEDICAL_INV= ?,MEDICAL_AMT= ?,MEDICAL_PAY= ?,MEDICAL_RMK= ?,MEDICAL_PRO=? WHERE ID= ?`;

console.log('ID',ID)


pool.query(sql,[Concession,Handling,BasicFee,Concession_handler,1,Flt_nbr,MTD_PAY,ACFT,MTOW,TTL_CREW,NAT_FLT,ARV_DTE,PROV,DEST,PAX_IN,PAX_OUT,CGO_IN,CGO_OUT,EA,ED,PKG,DLY_CODE,DLY_TIME,DLY_REASON,INF_IN_OUT,LIGHTING,COMMENT,PUSH_BACK,HANDLER,DSK_USED,CHK_IN_OPEN,CHK_IN_CLOSE,BOARD_OPEN,BOARD_CLOSE,DOORS_SHUT,COMMENT_CHK_IN,COMMENT_BOARD,INFO_GIVEN_PAX,
  PAX_REACTION,EXP_INCURRED,OTHER_GSE,OPERATOR,PROVIDER,COMMENT_CLEAN,COMMENT_RAMP,COMMENT_CGO,COMMENT_CATERING,EXP_AUTH_BY,COMMENT_CREW,WO_HNDL_INV,APT_FEES_INV,APT_FEES_AMT,APT_FEES_PAY,APT_FEES_RMK,TASPT_INV,TASPT_AMT,TASPT_PAY,TASPT_RMK,VIP_ARV_INV,VIP_ARV_AMT,VIP_ARV_PAY,VIP_ARV_RMK,VIP_ARV_PRO,VIP_DEP_INV,VIP_DEP_AMT,VIP_DEP_PAY,VIP_DEP_RMK,VIP_DEP_PRO,CATERING_INV,CATERING_AMT,CATERING_PAY,CATERING_RMK,CATERING_PRO,CREW_TRS_INV,CREW_TRS_AMT,CREW_TRS_PAY,CREW_TRS_RMK,CREW_TRS_PRO,PAX_TRS_INV,PAX_TRS_AMT,PAX_TRS_PAY,PAX_TRS_RMK,PAX_TRS_PRO,CREW_HTC_INV,CREW_HTC_AMT,CREW_HTC_PAY,CREW_HTC_RMK,CREW_HTC_PRO,PAX_HTC_INV,PAX_HTC_AMT,PAX_HTC_PAY,PAX_HTC_RMK,PAX_HTC_PRO,FUEL_INV,FUEL_AMT,FUEL_PAY,FUEL_RMK,FUEL_PRO,CGO_INV,CGO_AMT,CGO_PAY,CGO_RMK,CGO_PRO,TECH_INT_INV,TECH_INT_AMT,TECH_INT_PAY,TECH_INT_RMK,TECH_INT_PRO,LND_PMT_AMT,LND_PMT_RMK,TRVL_EXP_AMT,TRVL_EXP_RMK,PORTERS_AMT,PORTERS_RMK,PRNT_PAGES_AMT,PRNT_PAGES_RMK,PHONE_COM_AMT,PHONE_COM_RMK,OTHER_CHG1,OTHER_CHG1_INV,OTHER_CHG1_AMT,OTHER_CHG1_PAY,OTHER_CHG1_RMK,OTHER_CHG1_PRO,OTHER_CHG2,OTHER_CHG2_INV,OTHER_CHG2_AMT,OTHER_CHG2_PAY,OTHER_CHG2_RMK,OTHER_CHG2_PRO,CURRENC_EX_BUY,CURRENC_EX_SELL,SUPERVISION_ARV_BY,APPROVED_BY,RECEIVED_AT,SUP_CHARGE_NOTE_AMT,SUP_CHARGE_NOTE_INV,SUP_CHARGE_NOTE_RMK,EXCESS_BAG,VERSION,MOTIF,DATE_SUP,TRS_APT_INV,TRS_APT_PRO,TRS_APT_PAY,TRS_APT_AMT,TRS_APT_RMK,SUPERVISION_DEP_BY,GPU,GPU_TIME,ASU,flight_number_arv,OTHER_CHG3,OTHER_CHG3_INV,OTHER_CHG3_AMT,OTHER_CHG3_PAY,OTHER_CHG3_RMK,OTHER_CHG3_PRO,OTHER_CHG4,OTHER_CHG4_INV,OTHER_CHG4_AMT,OTHER_CHG4_PAY,OTHER_CHG4_RMK,OTHER_CHG4_PRO,CORVER_BY,CUTE_INV,CUTE_AMT,CUTE_PAY,CUTE_RMK,AMBU_NBR,WCH_NBR,UMNR_NBR,FAE_LINK,APT_FEES_LINK,CATERING_LINK,APT_TRANS_LINK,CREW_TRANS_LINK,PAX_TRANS_LINK,FUEL_LINK,CREW_HOTAC_LINK,PAX_HOTAC_LINK,CGO_FEES_LINK,TECH_INTER_LINK,FACTURETTES_LINK,TASPT_LINK,CUTE_LINK,QSR_LINK,ROJ_WO_LINK,IMOC_FORM_LINK,IMOC_HDLG_LINK,Service_Type,OTHER_CHG5,OTHER_CHG5_INV,OTHER_CHG5_AMT,OTHER_CHG5_PAY,OTHER_CHG5_RMK,OTHER_CHG5_PRO,MEDICAL_INV,MEDICAL_AMT,MEDICAL_PAY,MEDICAL_RMK,MEDICAL_PRO,ID],(err,data)=> {console.log(data)}); 

  // const sql=`UPDATE flight SET Concession=?,Handling=?,BasicFee=? WHERE ID= ?`;
  
  // console.log('ID',ID)
  
  
  // pool.query(sql,[Concession,Handling,BasicFee,ID],(err,data)=> {console.log(data)}); 
